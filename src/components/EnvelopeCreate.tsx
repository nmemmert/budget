...
export default function EnvelopeCreate({ onEnvelopeCreated, onCancel, accounts }: EnvelopeCreateProps) {
  const [name, setName] = useState('');
  const [allocated, setAllocated] = useState('');
  const [selectedColor, setSelectedColor] = useState(colorOptions[0].value);
  const [selectedAccount, setSelectedAccount] = useState(accounts[0]?.id || '');
  const [incomeAllocationType, setIncomeAllocationType] = useState<'percentage' | 'fixed'>('percentage');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setIncomeAllocationType('percentage');
    setIncomeAllocation('');
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Create New Envelope</h3>
